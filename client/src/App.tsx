...
export default function App() {
  ...
  const downloadSong = (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!url) {
      ...
      if (url.includes("https://www.tiktok.com/") || url.includes("/photo/") && url.includes("https://www.tiktok.com/") && url.includes("https://vm.tiktok.com/")) {
      ...
      <div className="drawer-content">
        <main className="flex flex-col w-screen h-screen items-center justify-center gap-4 p-4 md:p-6">
          <header className="w-full flex items-center justify-between">
            <Link className="text-3xl font-bold hover:text-primary-content/30 transition-colors duration-200 md:text-4xl" to="/">TikSlides.</Link>
          ...
          <h1 className="text-5xl font-bold md:text-7xl">TikSlides.</h1>
          <p className="md:w-[30rem] md:text-xl md:text-center">Convert <span className="font-bold">TikTok slideshows</span> into <span className="font-bold">one video</span>, in just <span className="font-bold">one click.</span></p>
          <form onSubmit={downloadSong} className="w-full flex flex-col gap-4 md:w-[34rem]">
            <div className="w-full flex items-center gap-3">
            <input type="url" placeholder="https://www.tiktok.com/@example/photo/021542" className="input input-bordered w-full placeholder:text-neutral/50" value={url} onChange={(e) => setUrl(e.target.value)}/>
